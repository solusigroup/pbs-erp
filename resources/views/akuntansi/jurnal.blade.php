            ...
            <div class="rounded-3xl border border-slate-800 bg-slate-900/60 overflow-hidden shadow-xl p-5 {{ !$j->is_posted ? 'ring-1 ring-amber-500/50 relative' : '' }}">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-3 mb-3 border-b border-slate-800/80 gap-2">
                    ...
                    <div class="flex items-center gap-3 text-xs text-slate-400">
                        ...
                        @if(auth()->user()->canMutate())
                            <button type="button" 
                                data-jurnal="{{ base64_encode(json_encode([
                                    'id_jurnal' => $j->id_jurnal,
                                    'no_transaksi' => $j->no_transaksi,
                                    'tanggal' => $j->tanggal ? $j->tanggal->format('Y-m-d') : date('Y-m-d'),
                                    'tipe_jurnal' => $j->tipe_jurnal,
                                    'sumber_referensi' => $j->sumber_referensi ?? '',
                                    'deskripsi' => $j->deskripsi,
                                    'is_posted' => (bool)$j->is_posted,
                                    'is_auto' => \App\Services\JurnalAutoService::isAutoGenerated($j),
                                    'details' => $j->details->map(function($d) {
                                        return [
                                ...
                                class="px-2.5 py-1 rounded-lg bg-blue-500/20 hover:bg-blue-500 hover:text-white text-blue-400 text-[11px] font-semibold border border-blue-500/30 transition flex items-center gap-1"
...
<script>
    ...
    const availableAccounts = {!! json_encode($akuns) !!};
    ...
    function openEditModal(target, tanggalFallback, deskripsiFallback) {
        let data;
        if (typeof target === 'object' && target !== null && target.dataset && target.dataset.jurnal) {
            try {
                ...
                alert('Gagal membaca data jurnal untuk diedit.');
                return;
            }
        } else if (typeof target === 'string' || typeof target === 'number') {
            data = {
                id_jurnal: target,
                no_transaksi: 'JU-' + target,
        ...
        document.getElementById('formEditJurnal').action = '/akuntansi/jurnal/' + data.id_jurnal;
        document.getElementById('editNoTransaksi').value = data.no_transaksi || '';
        document.getElementById('editNoTransaksiBadge').innerText = data.no_transaksi || '';
        document.getElementById('editTanggal').value = data.tanggal || '';
        document.getElementById('editTipeJurnal').value = data.tipe_jurnal || 'Umum';
        document.getElementById('editSumberReferensi').value = data.sumber_referensi || '';
